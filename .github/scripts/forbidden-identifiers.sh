...
ids=$(git grep -nIE '(^|[^0-9a-zA-Z_])([0-9a-f]{32}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})($|[^0-9a-zA-Z_])' -- ':!pnpm-lock.yaml' ':!test/' || true)
...
report "32-hex or UUID identifiers outside test/, which look like account, zone or database ids:" "$ids"
